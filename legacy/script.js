...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('project-plan-container')) {
        ...
        if (dashboardTitle && currentProjectName) {
            ...
            // タイトルタグの更新を新サービス名に変更
            document.title = `ダッシュボード | ${currentProjectName} | Local Ruler`;
        }
        ...
        <div class="phase-edit-group" data-phase-index="${pIndex}">
            ...
            <div class="milestones-container">
                ${phase.milestones.map((ms, mIndex) => `
                    <div class="input-with-button">
                        <input type="text" class="form-control milestone-input" value="${ms}" placeholder="タスクを入力">
                        <button class="btn-icon remove-milestone-btn" title="タスクを削除"><i class="fas fa-times"></i></button>
                    </div>
                `).join('')}
            </div>
            <button class="btn-link add-milestone-btn">＋ タスクを追加</button>
        </div>
    `;
    ...
    const handleDynamicClicks = (e) => {
        ...
        if (e.target.closest('.add-milestone-btn')) {
            ...
            container.insertAdjacentHTML('beforeend', `
                <div class="input-with-button">
                    <input type="text" class="form-control milestone-input" placeholder="タスクを入力">
                    <button class="btn-icon remove-milestone-btn" title="タスクを削除"><i class="fas fa-times"></i></button>
                </div>
            `);
        }
